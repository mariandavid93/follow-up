...
<script type="text/javascript">
	...
	app.controller('DeleteController', function($scope, $http) {
		$scope.deleteService = function() {
			$http.del(
					"http://localhost:8080/Hibernate/institution/delete/"
							+ window.location.search.substring(4,
									window.location.search.length) + "").then(
					function(response) {
						alert("Serviciul a fost sters cu succes.");
						redirect();
					})
	...
	<sec:authorize access="hasRole('ROLE_ADMIN')">
		<div ng-app="app" ng-controller="DeleteController" id="content">
			<div id="box">
				...
				<div id="boxContent">
					<input type="submit" value="Delete service" id="delete"
						ng-click="deleteService()" /> <a
						href="http://localhost:8080/Hibernate/homepage/update/service" id="back">Back</a>
					</form>
				</div>
			</div>
		</div>
